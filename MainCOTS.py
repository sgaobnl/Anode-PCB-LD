...
Last modified: 3/30/2021 2:17:51 PM
"""
...
if (test_runs&0x20 != 0x0 ):
    ...
    print "time cost = %.3f seconds"%(timer()-start)
    chk_path = ceruns.soft_triger_run(apa_oft_info, sgs=[1], tps=[3], map_en = True ) ###FPGA-DAC calibraion
    #chk_path = ceruns.soft_triger_run(apa_oft_info, sgs=[1], tps=[3], map_en = False )
    with open(logfile, "a+") as f:
        f.write( "%2X: Quick Checkout Test\n" %(test_runs&0x20) ) 
        f.write (ceruns.runpath + "\n" )
        f.write (ceruns.runtime + "\n" )
...
if (test_runs&0x08 != 0x0 ):
    ...
    #chk_path = ceruns.hw_triger_run(apa_oft_info, sgs=[1], tps=[3], map_en=True )
    with open(logfile, "a+") as f:
        f.write( "%2X: Quick Checkout Test\n" %(test_runs&0x20) ) 
        f.write (ceruns.runpath + "\n" )
        f.write (ceruns.runtime + "\n" )
